...
<body>
	<%
		...
		// 사용자가 쿼리 값을 임의 변경하여 유효하지 않은 값을 만들었다면 보정을 해줘야 한다.
		if(month==0){
			month = 12;
			year--;
		}
		...
		// 1일의 요일과 그 달의 마지막 날짜를 구해서 출력해보자
			...
			<%
				...
				for(LunarVO vo : list){
					...
					if(dayOfWeekNumber==6){// 토요일이면
						...
						+ vo.getLunarDate() + "(" + vo.getGanjiDateK() + ")"
						+ "</span></td>"); 
					}else if(dayOfWeekNumber==0){// 일요일이면
						out.println("<td>"
						+ vo.getSolarDate()
						+ "<br><span>"
						// 1일은 월을 찍는다.
