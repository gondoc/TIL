...
<%
	// 1개 분량의 글을 얻어온다.
	// incHit 가 1이면 조회수가 증가되고 0이면 증가되지 않는다. 
...
<script type="text/javascript">
	function resetForm(){
		$("#m").val(1); // 모드를 저장으로
		$("#cidx").val(0); // 저장에는 글 번호가 없다.
		$("#submitBtn").val("저장"); // 버튼의 제목 변경
		$("#cancelBtn").css("visibility","hidden") // 취소 버튼을 숨긴다. 
		$("#name").val(""); // 이름을 지운다.
	...
	function updateForm(idx){
		$("#m").val(2); // 모드를 수정으로
		$("#cidx").val(idx); // 저장에는 글 번호가 있다.
		$("#subMitBtn").val("수정"); // 버튼의 제목 변경
		$("#cancelBtn").css("visibility","visible") // 취소 버튼을 보인다. 
			...
			<td colspan="4" style="border:none;">
				<div>
					<form action="commentUpdateOk.jsp" method="POST">
						<%-- 댓글을 저장/수정/삭제하고 다시 view로 이동하기 때문에 아래 값을 숨겨서 가져가야한다. --%>
						...
						<input type="password" name="password" id="password" placeholder="비밀번호입력" required="required" tabindex="2">
						<input type="submit" value="저장" id="submitBtn" tabindex="4" class="btn btn-outline-success btn-sm" /> <br />
						<input type="button" value="취소" id="cancelBtn" style="visibility: hidden;" onclick="resetForm()" class="btn btn-outline-success btn-sm">
						<textarea rows="5" cols="80" style="margin-top: 10px; width: 800px;" id="content" name="content" required="required" tabindex="3"></textarea>
					</form>
				</div>		
				<hr />
				<%-- 댓글 리스트 출력 --%>
				...
				<c:if test="${not empty vo.commentList }">
					...
					<div style="border:1px solid gray; padding: 5px; margin-bottom: 10px; font-size: 11pt;">
						<%-- 상단 --%>
						<div style="border:1px solid gray; padding: 5px; background-color: silver;">
							<c:out value="${cvo.name }"/> 님이
							${cvo.ip }에서
							<fmt:formatDate value="${cvo.regDate }" pattern="yyyy년 MM월 dd일 (E)요일 hh:mm:ss"/> 에 남긴 글 
							
							<%-- 수정/삭제할 때 가지고 갈 내용을 숨겨서 놓자 --%>				
							<div style="display: none;" id="name${cvo.idx }">${cvo.name }</div>
							<div style="display: none;" id="content${cvo.idx }">${cvo.content }</div>
							
							<button class="btn btn-outline-success btn-sm" onclick="updateForm(${cvo.idx})" >수정</button>
							<button class="btn btn-outline-danger btn-sm" onclick="deleteForm(${cvo.idx})" >삭제</button>
						</div>	
						<%-- 내용 --%>
						<c:set var="c" value="${cvo.content }"/>
						<%-- 태그 무시 --%>
						<c:set var="content" value="${fn:replace(c,'<','&lt;') }"/>
						<%-- \n을 <br>로 변경 --%>
						<c:set var="content" value="${fn:replace(c,newLine,br) }"/>
						${c }
					</div>
