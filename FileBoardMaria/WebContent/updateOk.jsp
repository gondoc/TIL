...
<%-- 공통코드 삽입 --%>
<%-- 
	파일 첨부가 되므로 request.getParameter를 사용할 수 없다. 
	그래서 하나하나 받아줘야 한다.
...
<body>
	...
	if(isMultipart){
		...
		if(items!=null){ // 비어 있지 않다면 반복
			for(FileItem item : items){ // 반복
				if(item.isFormField()){ // 일반 폼 태그냐
					...
					if(sizeInBytes>0){ // 파일 크기가 있을때만
						String oriName = item.getName();	// 원본 파일이름
						// IE의 경우 원본파일 이름에 경로가 들어간다. 경로가 있으면 경로를 지워주어야 한다.
			        	...
			        	// 파일 저장
				        String saveName = UUID.randomUUID().toString(); // UUID : 겹치지 않는 ID를 만들어 준다. -- 저장 파일명으로 쓰자.
	...
	<%-- 모드 값에 따라 저장/수정/삭제를 서비스 클래스를 불러서 처리하면 된다. --%>
	...
	<%
		...
		// 위에서 자바스크립트를 사용했기 때문에 response.sendRedirect 사용 못 함.
		// 페이지 이동을 자바스크립트로 처리해야 한다.
		out.println(query); // 삭제 성공하면 목록으로 이동
