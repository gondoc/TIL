	...
	<table>
		...
		<tr>
			<td style="background-color: skyblue;">
				...
				<%=vo.getRegDate() %>
				<%-- 수정/삭제 버튼을 달자 --%>
				<button class="btn btn-primary btn-sm" onclick="location.href='update.jsp?p=${pv.currentPage}&s=${pv.pageSize }&b=${pv.blockSize }&idx=<%=vo.getIdx()%>'">수정</button>
				<button class="btn btn-primary btn-sm" onclick="location.href='delete.jsp?p=${pv.currentPage}&s=${pv.pageSize }&b=${pv.blockSize }&idx=<%=vo.getIdx()%>'">삭제</button>
			</td>
		</tr>
		<tr>
			<td>
				<%
					...
					// 엔터가 먹지 않아 줄바꿈이 되지 않는다. 엔터를 br태그로 변경처리해준다.
		...
		%>
		<tr>
			<td class="page_list">${pv.pageInfo }</td>
		</tr>
		<tr>
			<td class="sub_title">
				<%-- 글쓰기 버튼을 달자! --%>
				<button class="btn btn-primary" onclick="location.href='insert.jsp?p=${pv.currentPage}&s=${pv.pageSize }&b=${pv.blockSize }'">흔적 남기기</button>
			</td>
		</tr>
	</table>
</body>
